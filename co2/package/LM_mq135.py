...
def __get_resistance():
    ...
    if __ADC is None:
        if 'esp8266' in platform:
            ...
            __ADC_PROP = (1023, 3.6)
    value = __ADC.read()
    if value <= 0 or value >= __ADC_PROP[0]:
        raise ValueError("invalid ADC reading: {}".format(value))
...
def __get_corrected_resistance(temperature, humidity):
    ...
    factor = __get_correction_factor(temperature, humidity)
    if factor <= 0:
        raise ValueError("invalid temperature/humidity correction factor")
    return __get_resistance() / factor
...
def raw_measure_mq135():
    ...
    try:
        ...
        return "raw_measure_mq135 ERROR: {}".format(e)


def measure_mq135(temperature=None, humidity=None):
    """
    Estimated CO2-equivalent gas concentration in parts per million (ppm).

    MQ-135 is a broad air-quality sensor, not a selective CO2 sensor. Results
    depend on calibration, warm-up, supply stability, and interfering gases.
    -> 1ppm = 0.0001% gas.
    ...
    :param temperature int: temperature compensation in Celsius
    :param humidity int: relative humidity compensation in percent
    :return str: verdict
    """
