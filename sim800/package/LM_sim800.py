...
class Sim800:
    ...
    def __init__(self, pin_code, tx_pin=16, rx_pin=17, ri_pin=23):
        self.uart_no = 1
...
def load(pin_code=None):
    ...
    """
    if Sim800.INSTANCE is None:
        Sim800.INSTANCE = Sim800(pin_code)
    ...
    for value in uart_lines:
        if 'RING' in value:
            ...
            if _ring_seen:
                ...
                console(f"Stale CLIP ignored (no RING): {value}")
        elif '+CMTI' in value:
            try:
...
# Track whether we've seen RING before CLIP to avoid stale call events
_ring_seen = False
